...
//Validate Brand page tealium profile ('page_name' & 'program name' different in some site. Hence validating 'page_name' & 'program name' is not null)
def utag = WebUI.executeJavaScript('return window.utag_data', null)

assert (utag['page_type']).equals('landing-page')
assert (utag['partner_name']).equals(tealiumProfile)
assert (utag['site_type']).equals('Landing Page')
assert !(utag['page_name']).equals('')
assert (utag['is_conversion']) == 0
assert (utag['is_landing_page']) == 1
assert !(utag['program_name']).equals('')
assert (utag['page_category']).equals('Landing Page')
assert (utag['site_section']).equals('Landing Page')
assert (utag['site_subsection1']).contains('Brand')
...
assert (utag['is_landing_page']) == 1
//Checking the program name doesnt contain the default 'partner-brand' value
