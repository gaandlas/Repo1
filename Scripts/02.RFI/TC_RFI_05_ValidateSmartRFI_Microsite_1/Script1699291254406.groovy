...
if (executionProfile != "default") {
	...
	singleProgramURL = GlobalVariable.sigleProgramURL
}
...
//Navigate to the program page
 WebUI.callTestCase(findTestCase('Test Cases/01.CommonTestSteps/LaunchSite'), [('url') : url + singleProgramURL], FailureHandling.STOP_ON_FAILURE)
 
 //Get smart rfi cookie value
 WebDriver driver = DriverFactory.getWebDriver()
 def uuid = driver.manage().getCookieNamed('smart_rfi_uuid').getValue()
 assert !(uuid.equals(''))

 //In some legacy Wordpress sites, program page RFI form does not contains program dropdown. Hence default program selection tested only form the sites with program drop-down.
 if (WebUI.verifyElementPresent(findTestObject('Pages/RFIPage/Program'), 5, FailureHandling.OPTIONAL)) {
	 //Get program selected by default
	 String program= WebUI.getAttribute(findTestObject('Object Repository/Pages/RFIPage/Program'), 'value')
	 
	 //Validate smartrfi cookie value and program selected by default is equal
	 assert (uuid.equals(program))
 }
 
 //In some legacy Wordpress sites, program page RFI form is a Two - Step Form.
 if (WebUI.verifyElementVisible(findTestObject('Pages/RFIPage/NextButton'), FailureHandling.OPTIONAL)) {
	 //Fill Two Step Form
	 WebUI.callTestCase(findTestCase('Test Cases/01.CommonTestSteps/FillRFIForm_TwoStepForm'),[('firstName') : firstName, ('lastName') : lastName, ('email') : email, ('country') : country, ('state') : state, ('phone') : phoneNumber, ('zip') : zipcode, ('smsConsentCheck') : smsConsentCheck, ('militaryCheck') : militaryCheck, ('isAffiliate') : isAffiliate,('tealiumProfile') : tealiumProfile,('formType') : formType], FailureHandling.STOP_ON_FAILURE)
