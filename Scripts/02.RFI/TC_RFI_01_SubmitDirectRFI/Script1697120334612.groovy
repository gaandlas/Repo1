...
/***
 * Submit a lead as a direct lead + with SMS Consent check selected + Country selected as 'United State of America' with a State (if fields available) + Military checkbox selected (if field available)
 * 
 * ***/ 
...
const programSelector = '#fr-programs'; // Replace with the actual selector of your program dropdown
  const programExists = await page.isVisible(programSelector);

  let programUuid = '';
  if (programExists) {
    await page.selectOption(programSelector, { index: 1 }); // Select the first option (example)
    programUuid = await page.getAttribute(programSelector, 'value');
  } else {
    // Get the program UUID dynamically or from a predefined variable
